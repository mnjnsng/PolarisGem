...
class WaypointPIDControl(object):
    ...
    def start_drive(self):
        ...
        while not rospy.is_shutdown():
            ...
            gvcx = self.path_points_x[self.goal] - cur_x
            gvcy = self.path_points_y[self.goal] - cur_y
            goal_x_veh_coord = gvcx*np.cos(cur_yaw) + gvcy*np.sin(cur_yaw)
            goal_y_veh_coord = gvcy*np.cos(cur_yaw) - gvcx*np.sin(cur_yaw)

            # TODO: define your feedback value
            goal_angle = np.arctan2(goal_y_veh_coord,goal_x_veh_coord)
            cur_feedback_val = goal_angle
